...
describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    let length = prodItems.length;

    for(let i = 0; i < length; i++ ) {

      //console.log(`Checking product item ${(i + 1)}/${length}`);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++ ) {
      ...
      if(innerText['_remoteObject'].value != 'Remove from Cart') { refreshWorked = false; }
      
    }

    let count = await page.$('#cart-count');
    count = await count.getProperty('innerText');
    if(count['_remoteObject'].value != '20') { refreshWorked = false; }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const storage = await page.evaluate(() => {
      return Promise.resolve(window.localStorage.cart);
    });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++ ) {
      ...
      button = await shadowRoot.$('button');
      innerText = await button.getProperty('innerText');
      if(innerText['_remoteObject'].value != 'Add to Cart') { refreshWorked = false; }
      
    }

    let count = await page.$('#cart-count');
    count = await count.getProperty('innerText');
    if(count['_remoteObject'].value != '0') { refreshWorked = false; }
